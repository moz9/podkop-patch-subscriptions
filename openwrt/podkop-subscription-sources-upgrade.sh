...
sh -n "$source_runtime"
if grep -Fqx '# subscription_source_actions_v1' "$target" && grep -Fqx '# subscription_isolated_probe_v1 end' "$target"; then
...
names='append_subscription_item filter_working_subscription_proxy_links refresh_subscription_cache load_subscription_proxy_links_for_section apply_subscription_exclusions_to_cached_links set_subscription_sections_enabled subscription_speedtest_stop subscription_update subscription_update_json'
...
$0=="get_subscription_sources)" || $0=="subscription_ping)" { skip_case=1; next }
