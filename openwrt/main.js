...
async function callBaseMethod(method, args = [], command = "/usr/bin/podkop", timeout = 15e3) {
  ...
  return {
    success: false,
    error: (response.stderr || "").trim() || (response.code ? `Command failed (${response.code})` : "")
  };
}
...
((Podkop2) => {
  ...
  ((AvailableMethods2) => {
    ...
    AvailableMethods2["SET_SUBSCRIPTION_SECTIONS_ENABLED"] = "set_subscription_sections_enabled";
...
var PodkopShellMethods = {
  ...
  setSubscriptionSectionsEnabled: async (changesBySection) => callBaseMethod(
    Podkop.AvailableMethods.SET_SUBSCRIPTION_SECTIONS_ENABLED,
    [
      JSON.stringify({
...
function hasFakeIPStatus(value) {
  return typeof value === "object" && value !== null && typeof value.fakeip === "boolean";
}
function getComparableIP(value) {
  if (typeof value !== "object" || value === null) return "";
  const IP = value.IP;
  return typeof IP === "string" ? IP.trim() : "";
}
function getDnsAvailability(value) {
  if (typeof value !== "object" || value === null) return void 0;
  const dnsAvailable = value.dnsAvailable;
  return typeof dnsAvailable === "boolean" ? dnsAvailable : void 0;
}
async function runFakeIPCheck() {
  ...
  const [routerFakeIPResponse, checkFakeIPResponse, checkIPResponse] = await Promise.all([
    PodkopShellMethods.checkFakeIP(),
    RemoteFakeIPMethods.getFakeIpCheck(),
    RemoteFakeIPMethods.getIpCheck()
  ]);
  const routerProbeAvailable = routerFakeIPResponse.success && hasFakeIPStatus(routerFakeIPResponse.data);
  const routerDnsAvailable = routerFakeIPResponse.success ? getDnsAvailability(routerFakeIPResponse.data) : void 0;
  const browserProbeAvailable = checkFakeIPResponse.success && hasFakeIPStatus(checkFakeIPResponse.data);
  const browserIP = checkFakeIPResponse.success ? getComparableIP(checkFakeIPResponse.data) : "";
  const proxyIP = checkIPResponse.success ? getComparableIP(checkIPResponse.data) : "";
  const ipProbeAvailable = Boolean(browserIP && proxyIP);
  const checks = {
    router: routerProbeAvailable && routerFakeIPResponse.data.fakeip === true,
    browserFakeIP: browserProbeAvailable && checkFakeIPResponse.data.fakeip === true,
    differentIP: browserProbeAvailable && checkFakeIPResponse.data.fakeip === true && ipProbeAvailable && browserIP !== proxyIP
  };
    ...
    items: [
      {
        ...
        key: !routerProbeAvailable ? _("Router FakeIP probe is unavailable") : routerDnsAvailable === false ? _("Router DNS did not return an answer") : checks.router ? _("Router DNS is routed through sing-box") : _("Router DNS is not routed through sing-box"),
        value: ""
      },
      {
        state: checks.browserFakeIP ? "success" : "warning",
        key: !browserProbeAvailable ? _("Browser FakeIP probe is unavailable") : checks.browserFakeIP ? _("Browser is using FakeIP correctly") : _(
      ...
      ...insertIf(checks.browserFakeIP, [
        {
          state: !ipProbeAvailable ? "warning" : checks.differentIP ? "success" : "error",
          key: !ipProbeAvailable ? _("Proxy routing probe is unavailable") : checks.differentIP ? _("Proxy traffic is routed via FakeIP") : _("Proxy traffic is not routed via FakeIP"),
          value: ""
        }
      ])
    ]
...
@media (max-width: 800px) {
    ...
    .pdk_diagnostic-page__right-bar__wiki .pdk-partial-button {
        width: 100%;
    }
...
function getSourceGroups(section) {
  const sources = section.sources || [];
  if (sources.length) return sources.map(source => ({...source,
    items: section.items.filter(item => item.sourceIds?.length ? item.sourceIds.includes(source.id) : (item.sourceIndex || 1) === source.sourceIndex),
    firstItemIndex: 0
  }));
  const groups = [];
  section.items.forEach((item,index) => {
    const sourceIndex = item.sourceIndex || 1;
    let group = groups.find(group => group.sourceIndex === sourceIndex);
    if (!group) { group = {sourceIndex,enabled:true,items:[],firstItemIndex:index}; groups.push(group); }
    group.items.push(item);
  });
  return groups;
}
function getEffectiveSourceEnabled(pendingChanges, sectionCode, source) {
...
function getSourceSummary({
  ...
  if (group.error) parts.push(group.error === "download_failed" ? "Сервер подписки недоступен — сохранён предыдущий список" : "Сервер вернул неподдерживаемый формат подписки");
  return parts.join(" | ");
}
...
function renderRow({
  ...
  return E(
    ...
    [
      ...
      E(
        "td",
        { "data-label": _("Config"), "data-title": _("Config"), title: getItemName(item, index) },
        getItemName(item, index)
      ),
      E(
        "td",
        { "data-label": _("Protocol"), "data-title": _("Protocol") },
        (item.protocol || _("Unknown")).toUpperCase()
      ),
      E(
        "td",
        { "data-label": _("Transport"), "data-title": _("Transport") },
        item.transport || _("Default")
      ),
      E(
        "td",
        { "data-label": _("Ping"), "data-title": _("Ping") },
        renderLatency()
      ),
      E(
        "td",
        { "data-label": _("Speed"), "data-title": _("Speed") },
        renderSpeed()
      ),
      E(
        "td",
      ...
      E(
        ...
        group.items.map(
          (item, index) => renderRow({
            section,
            item: {...item, subscriptionDisabled: !getEffectiveSourceEnabled(pendingChanges, section.code, group)},
...
function renderSourceGroup({
  ...
  return E(
    ...
    [
      E("div", {class:"pdk_subscriptions-page__source-controls"}, [
      E("label", {class:"pdk_subscriptions-page__source-toggle", title:"Использовать эту подписку для рабочего трафика"}, [
...
function getEnabledSupportedItems(section) {
  return section.items.filter((item) => item.supported);
}
...
async function handleApply() {
  ...
  try {
    ...
    if (!result.success || !result.data || result.data.success !== true) {
      throw new Error(
        result.success && result.data?.state === "rollback_failed" ? "rollback_failed" : result.success ? result.data?.error : result.error
      );
    }
  ...
  try {
    ...
    for (const section of sections) {
      ...
      for (const item of enabledItems) {
        const result = await PodkopShellMethods.pingSubscription(section.code, item.id);
        latencyByRow[getRowId2(section.code, item.id)] = result.success && result.data?.success ? result.data.latencyMs : 0;
        store.set({subscriptionItemsWidget:{...store.get().subscriptionItemsWidget,latencyByRow:{...latencyByRow}}});
      }
    }
    store.set({
      subscriptionItemsWidget: {
...
#cbi-podkop-subscriptions .cbi-value-field,
#cbi-podkop-subscriptions .cbi-value,
#subscriptions-sections-grid {
    width: 100%; min-width: 0; margin-left: 0; box-sizing: border-box;
}
...
.pdk_subscriptions-page__toggle-target {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    min-width: 44px;
...
@media (max-width: 800px) {
    ...
    .pdk_subscriptions-page__table thead {
        display: none;
    }

    .pdk_subscriptions-page__table tbody {
        display: grid;
        grid-template-columns: minmax(0, 1fr);
        gap: 8px;
        padding: 8px;
    ...
    .pdk_subscriptions-page__table tr {
        display: grid;
        grid-template-columns: minmax(0, 1fr);
        min-width: 0;
        border: 1px var(--background-color-low, lightgray) solid;
    ...
    .pdk_subscriptions-page__table td::before {
...
@media (max-width: 800px) {
    ...
    #view .cbi-page-actions .cbi-button {
        flex: 1 1 auto;
