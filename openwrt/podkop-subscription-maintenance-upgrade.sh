...
if ! grep -q "restore_community_subnet_cache_v1" "$target" 2>/dev/null; then
	community_subnet_functions="$(mktemp)"
	cat > "$community_subnet_functions" <<'COMMUNITY_SUBNET_EOF'
get_community_subnet_cache_path() {
    local service="$1"
...
restore_cached_community_subnet_list_handler() {
    local service="$1"
    local cache_path restore_community_subnet_cache_v1

    restore_community_subnet_cache_v1=1
    cache_path="$(get_community_subnet_cache_path "$service")"
    [ -s "$cache_path" ] || return 0
...
COMMUNITY_SUBNET_EOF

	download_replacement="$(mktemp)"
	cat > "$download_replacement" <<'DOWNLOAD_REPLACEMENT_EOF'
    if ! download_to_file "$URL" "$tmpfile" "$http_proxy_address" || [ ! -s "$tmpfile" ]; then
        local cache_path
        cache_path="$(get_community_subnet_cache_path "$service")"
        if [ -s "$cache_path" ]; then
            log "Download $service list failed, using cached subnet list" "warn"
            cp "$cache_path" "$tmpfile"
        else
            log "Download $service list failed" "error"
            return 1
        fi
    else
        cache_community_subnet_list "$service" "$tmpfile"
    fi
DOWNLOAD_REPLACEMENT_EOF

	awk -v funcs="$community_subnet_functions" -v repl="$download_replacement" '
	BEGIN {
		while ((getline line < funcs) > 0) {
			functions_text = functions_text line "\n"
		}
		while ((getline line < repl) > 0) {
			replacement_text = replacement_text line "\n"
		}
		skip_download_block = 0
		inserted_cache_dir = 0
		inserted_functions = 0
		inserted_restore_call = 0
	}

	$0 == "SUBSCRIPTION_CACHE_DIR=\"/etc/podkop/subscriptions\"" && !inserted_cache_dir {
		print
		print "COMMUNITY_SUBNET_CACHE_DIR=\"$TMP_SING_BOX_FOLDER/community-subnets\""
		inserted_cache_dir = 1
		next
	}

	$0 == "    create_nft_rules" && !inserted_restore_call {
		print
		print "    if [ \"$PODKOP_SKIP_LIST_UPDATE\" = \"1\" ]; then"
		print "        config_foreach restore_cached_community_subnet_lists \"section\""
		print "    fi"
		inserted_restore_call = 1
		next
	}

	$0 == "# Main funcs" && !inserted_functions {
		printf "%s", functions_text
		print
		inserted_functions = 1
		next
	}

	$0 == "    download_to_file \"$URL\" \"$tmpfile\" \"$http_proxy_address\"" && !skip_download_block {
		printf "%s", replacement_text
		skip_download_block = 1
		next
	}
	...
	{ print }
	' "$target" > "$tmp" || {
		rm -f "$community_subnet_functions" "$download_replacement" "$tmp"
		exit 1
	}
	cat "$tmp" > "$target"
	rm -f "$community_subnet_functions" "$download_replacement" "$tmp"
