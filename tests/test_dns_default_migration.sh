...
for installer in i openwrt/install.sh; do
    grep -q '^INSTALL_MARKER="PODKOP_SUBSCRIPTIONS_PATCH_VERSION=20260911-deferred-subscriptions-v1"$' "$installer" ||
    ...
    grep -q '^LUCI_MODULE_NAMESPACE="podkop_patch_20260911_deferred_subscriptions_v1"$' "$installer" ||
        fail_test "$installer LuCI namespace does not match the current release"
done

[ "$(jq -r '.patchVersion' openwrt/update-manifest.json)" = '20260911-deferred-subscriptions-v1' ] ||
